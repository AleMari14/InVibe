  ...
  const fetchEvents = useCallback(async () => {
    try {
      ...
      if (Array.isArray(data)) {
        // Assicuriamoci che ogni evento abbia le proprietà necessarie
        const validatedEvents = data.map((event) => ({
          ...event,
          images: Array.isArray(event.images) ? event.images : [],
          location: event.location || "Posizione non specificata",
          title: event.title || "Titolo non disponibile",
          category: event.category || "generale",
          price: typeof event.price === "number" ? event.price : 0,
          totalSpots: typeof event.totalSpots === "number" ? event.totalSpots : 0,
          availableSpots: typeof event.availableSpots === "number" ? event.availableSpots : 0,
          rating: typeof event.rating === "number" ? event.rating : 0,
          reviewCount: typeof event.reviewCount === "number" ? event.reviewCount : 0,
          amenities: Array.isArray(event.amenities) ? event.amenities : [],
        }))

        setEvents(validatedEvents)
        const featured = [...validatedEvents]
  ...
  const filteredEvents = events.filter(
    (event) =>
      ...
      (event.category &&
        ...
        (isDefaultDate ||
          ((!dateFrom || new Date(event.dateStart || 0) >= new Date(dateFrom)) &&
            (!dateTo || new Date(event.dateStart || 0) <= new Date(dateTo))))),
  )
                      ...
                      <CardContent className="p-4 flex-grow flex flex-col justify-between">
                        <div>
                          ...
                          <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400 mb-2">
                            ...
                            <span>{event.location && event.location.split(",")[0]}</span>
                          </div>
                        </div>
